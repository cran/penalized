...
The main algorithm for L1 penalized estimation (lasso and elastic net) that used in this package is documented in \cite{Goeman2010}. It has been combined with ideas from \cite{Eilers2001} and \cite{Houwelingen2006} for efficient L2 penalized estimation.
...
penalty(fit)
@
The \Rfunction{loglik} function gives the loglikelihood without the penalty, and the \Rfunction{penalty} function gives the fitted penalty, i.e. for L1 \Robject{lambda1} times the sum of the absolute values of the fitted coefficients.
...
It is also possible to constrain only part of the regression coefficients to be positive by giving the \Rfunarg{positive} argument as a logical vector.
