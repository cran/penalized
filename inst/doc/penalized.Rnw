...
fit <- penalized(Surv(time,event), nki70[,10:11], data=nki70, lambda2=1)
fit <- penalized(Surv(time,event)~DIAPH3+NUSAP1, data=nki70, lambda2=1)
@
...
If the covariates are not on the same scale in a natural way, it is advisable to standardize them. The function argument \Rfunarg{standardize} (default: \Robject{FALSE}) standardizes the covariates to unit second central moment before applying penalization. This standardization makes sure that each covariates is affected more or less equally by the penalization.
...
The penalized function returns a \Rclass{penfit} object, from which useful information can be extracted.

<<extract>>=
coefficients(fit)       # the fitted regression coefficients
residuals(fit)          # the martingale residuals
fitted.values(fit)      # the fitted relative risks
basehaz(fit)            # the baseline hazard
@

See \Robject{help(penfit)} for more information.


\subsection{unpenalized covariates}

In some situations not all covariates are subject to a penalty. Any additional covariates that should be included in the model without being penalized can be specified separately. using the third function argument (\Rfunarg{unpenalized}). For example
...
If some of the factors included in the \Rclass{formula} object \Rfunarg{penalized} are \Rclass{factor}, these are automatically made into dummy variables, as in \Rfunction{glm} and \Rfunction{coxph}, but in a special way that is more appropriate for penalized regression.
...
Ordered factors are turned into dummy variables that code for the difference between successive levels (one dummy less than the number of levels). L2 penalization on such factors therefore leads to small successive differences; L1 penalization leads to ranges of successive levels with identical effects.
...
\section{Cross-validation and optimization}
...
It accepts the same arguments as \Rfunction{penalized} (except \Rfunarg{steps}: see \Rfunction{profL1} below) as well as the \Rfunarg{fold} argument. This will usually be a single number $k$ to indicate $k$-fold cross-validation. In that case, the allocation of the subjects to the folds is random. Alternatively, \Rfunarg{fold} can give the precise allocation of the subjects into the folds by giving a vector of the length of the number of subjects with values form 1 to $k$, each indicating the fold of the corresponding subject. The default is to do leave-one-out cross-validation.
...
\item[\Robject{predictions}] the prediction made on each left-out subject. The format depends on the model used. In logistic regression this is just a vector of probabilities. In the Cox model this is a collection of predicted survival curves (a \Rclass{breslow} object). In the linear model this is a collection of predicted means and predicted standard deviations (the latter are the maximum likelihood estimates of $\sigma^2$).
...
The allocation of the subjects into cross-validation folds is done only once, so that all cross-validated likelihoods are done with the same allocation. This makes the cross-validated log likelihoods more comparable. As in \Rfunction{cvl} the allocation is returned in \Robject{fold}.
