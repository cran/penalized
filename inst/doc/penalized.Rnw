...
This short note explains the use of the \Rpackage{penalized} package. The package is designed for penalized estimation in generalized linear models. The lasso and elastic net algorithm that it implements is described in \cite{Goeman2009}. A preprint can be downloaded from \verb:www.msbi.nl/goeman: Please cite that paper if you use this package.
